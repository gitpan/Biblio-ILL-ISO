package Biblio::ILL::ISO::CheckedIn;
...
Biblio::ILL::ISO::CheckedIn is a derivation of the abstract 
Biblio::ILL::ISO::ISO object, and handles the Checked-In message type.
...
Checked-In message from a file (followed by a call to
...
To read a message from a file, use the following:
    ...
    my $href = $msg->read("msg_11.checked-in.ber");
...
=head2 set_date_checked_in($dci)

 Sets the message's date-checked-in.
...
=cut
sub set_date_checked_in {
    my $self = shift;
    my ($parm) = shift;

    if (ref($parm) eq "Biblio::ILL::ISO::ISODate") {
	$self->{"date-checked-in"} = $parm;
    } else {
	$self->{"date-checked-in"} = new Biblio::ILL::ISO::ISODate($parm);
    }
